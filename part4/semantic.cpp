...
namespace VSOP
{
    ...
    Type SemanticAnalyzer::checkClass(ClassAst *classAst)
    {
        ...
        for (auto field : classAst->fields)
        {
            ...
            if (fieldFound)
            {
                reportError(7, 5, "redefinition of field " + field->name + " (first defined at 2:5 in parent class " + currentClassName + ")");
                continue;
            }
            ...
            for (size_t i = 0; i < method->formals.size(); i++)
            {
                ...
                if (formalNames.find(formal->name) != formalNames.end())
                {
                    ...
                    for (size_t j = 0; j < i; j++)
                    {
                        ...
                        {
                            reportError(2, 35, "redefinition of argument " + formal->name + " (first defined at 2:14).");
                            hasDuplicate = true;
                            break;
                        }
                    }
