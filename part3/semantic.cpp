...
namespace VSOP
{
    ...
    bool SemanticAnalyzer::analyze(ProgramAst *program)
    {
        ...
        for (auto classAst : program->classes)
        {
            ...
            {
                reportError(1, 1, "redefinition of class " + classAst->name);
                continue;
            }
        ...
        for (auto classAst : program->classes)
        {
            ...
            {
                reportError(1, 1, "use of undefined type " + classAst->parent);
            }
        }
    ...
    Type SemanticAnalyzer::checkClass(ClassAst *classAst)
    {
        ...
        for (auto field : classAst->fields)
        {
            ...
            if (fieldFound)
            {
                reportError(1, 1, "redefinition of field " + field->name + " (first defined at 1:1 in parent class " + currentClassName + ")");
                continue;
            }
            ...
            if (field->initExpr)
            {
                ...
                {
                    reportError(1, 1, "expected type " + fieldType.toString() + ", but found type " + initType.toString());
                }
            }
        }
        ...
        for (auto method : classAst->methods)
        {
            ...
            {
                reportError(1, 1, "redefinition of method " + method->name + ", first defined at " + std::to_string(currentClass->line) + ":" + std::to_string(currentClass->column));
                continue;
            }
            ...
            for (size_t i = 0; i < method->formals.size(); i++)
            {
                ...
                if (formalNames.find(formal->name) != formalNames.end())
                {
                    ...
                    for (size_t j = 0; j < i; j++)
                    {
                        ...
                        {
                            reportError(1, 1, "redefinition of argument " + formal->name + " (first defined at 1:1).");
                            hasDuplicate = true;
                            break;
                        }
                    }
                ...
                if (formal->type == "unit")
                    paramType = Type::Unit();
                else if (formal->type == "bool")
                    paramType = Type::Bool();
                else if (formal->type == "int32")
                    paramType = Type::Int32();
                else if (formal->type == "string")
            ...
            else if (method->returnType == "int32")
        ...
        for (auto method : classAst->methods)
        {
            ...
            if (method->body)
            {
                ...
                {
                    reportError(1, 1, "expected type " + methodSig.returnType.toString() + ", but found type " + bodyType.toString());
                }
            }
    ...
    Type SemanticAnalyzer::checkIf(IfExprAst *ifExpr, const Type &expectedType)
    {
        if (expectedType.getKind() == Type::Kind::ERROR)
        {
            ...
            if (thenType != elseType)
            {
                ...
                reportError(1, 1, "expected type " + thenType.toString() + ", but found type " + elseType.toString());
            }
    ...
    Type SemanticAnalyzer::checkLet(LetExprAst *letExpr, const Type &expectedType)
    {
        ...
        if (letExpr->type == "unit")
            declaredType = Type::Unit();
        else if (letExpr->type == "bool")
            declaredType = Type::Bool();
        else if (letExpr->type == "int32")
            declaredType = Type::Int32();
        ...
        if (letExpr->initExpr)
        {
            ...
            {
                reportError(1, 1, "expected type " + declaredType.toString() + ", but found type " + initType.toString());
            }
        }
    ...
    Type SemanticAnalyzer::checkAssign(AssignExprAst *assign, const Type &expectedType)
    {
        ...
        {
            reportError(1, 1, "expected type " + varType.toString() + ", but found type " + exprType.toString());
        }
    ...
    Type SemanticAnalyzer::checkCall(CallExprAst *call, const Type &expectedType)
    {
        ...
        {
            reportError(1, 1, "method " + call->methodName + " expects " + std::to_string(methodSig->parameterTypes.size()) + " arguments, but got " + std::to_string(call->arguments.size()));
            return Type::Error();
        }
        ...
        for (size_t i = 0; i < call->arguments.size(); i++)
        {
            ...
            {
                reportError(1, 1, "expected type " + methodSig->parameterTypes[i].toString() + ", but found type " + argType.toString());
            }
        }
    ...
    bool SemanticAnalyzer::isSubtype(const Type &subtype, const Type &supertype)
    {
        ...
        if (subtype.getKind() == Type::Kind::CLASS && supertype.getKind() == Type::Kind::CLASS)
        {
            ...
            {
                if (currentClass == supertype.getClassName())
                    return true;
            ...
            if (currentClass == "Object" && supertype.getClassName() == "Object")
                return true;
        }
        return false;
    }
    ...
    std::string SemanticAnalyzer::getCommonSupertype(const Type &type1, const Type &type2)
    {
        ...
        if (type1.getKind() == Type::Kind::CLASS && type2.getKind() == Type::Kind::CLASS)
        {
            ...
            std::string class1 = type1.getClassName();
            std::string class2 = type2.getClassName();
            ...
            // Find the least common ancestor by traversing up the hierarchy
            std::set<std::string> ancestors1;
            std::string current = class1;
            ...
            // If we reach Object, return Object
            return "Object";
        }
        return "Object"; // Default fallback for non-class types
    }
